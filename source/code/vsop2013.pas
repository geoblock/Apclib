...
// jul 2020: For reasons of max bundle size and load speed om mobile, oMAR adopted a custom binary format
...
type
  // TVector3D_D - Double 3D vector type record (RTL has TVector3D as Single)
  // and not long enough to hold planet coordinate calculations
  TVector3D_D = record
    x, y, z: Double;
    ...
    function getLatLonRadius(var aLat, aLon, aRadius: Double): boolean;
    // convert cartesian x,y,z ---> lat,lon,radius ( spherical coordinates )
  end;
...
type
  ...
  T_VSOP2013_binaryRec = packed record // rec len = 7840 bytes
    case boolean of // <----------------- don't mess with this. file records
      true:
        (t1, t2, delta: Double; // header ( 1st record )
          nintv, ncoef: integer;
          loc: array [1 .. 3, 1 .. 9] of integer;
        );
      false:
        (dj1, dj2: Double; // each rec corresponds to a 32 period ( there are 17122 of these )
  ...
  // VSOP2013 period as 32.0 days
  T_VSOP2013_Period = class
  private
  public
  ...
  public
    ...
    function Read_ASCII_File(const aFilename: String): boolean;
    // read vsop2013 ASCII file to memory
    procedure Threaded_Read_ASCII_File(const aFilename: String);
    procedure Threaded_ReadBinaryFile(const aFilename: String);
    function Calculate_coordinates(ip: integer; const jde: Double;
      var Position, Speed: TVector3D_D): boolean;
    function GetMetadata: String;
    property OnLoadProgress: T_VSOP2013_LoadProgress read fOnLoadProgress write fOnLoadProgress;
    property OnLoadTerminate: TNotifyEvent read fOnLoadTerminate write fOnLoadTerminate;
    // threaded loat terminate
  end;
...
// lat,lon in degrees, radius in au
const
  rad2deg = 180 / Pi;
begin
  ...
  begin
    ...
    Result := false; // zero radius --> no lat,lon
end;
...
begin
  ...
  begin
    ...
    for j := 1 to NUM_COEFS do
      aRec.coef[j] := aPeriod.coef[j]; // 978 coefs
    write(F, aRec);
  end;
  CloseFile(F);
  Result := true;
...
function T_VSOP2013_File.Read_ASCII_File(const aFilename: String): boolean;
// read vsop2013 ASCII file to memory
var
  InFile: Textfile;
  S, ss: String;
...
begin
  ...
  begin
    ...
    try
      ...
      idf := StrToIntDef(S, 0);
      Readln(InFile, S);
      t1 := StrToFloatDef(S, 0);
      Readln(InFile, S);
      t2 := StrToFloatDef(S, 0);
      Readln(InFile, S);
      delta := StrToFloatDef(S, 0);
      Readln(InFile, S);
      nintv := StrToIntDef(S, 0); // number of intervals in this file
      Readln(InFile, S);
      ncoef := StrToIntDef(S, 0);
...
begin
  ...
  begin
    ...
    l := aFN.Length;
    if (l > 20) then
      aFN := '...' + Copy(aFN, l - 20, MAXINT); // long filename...keep last part

    Result := 'file:' + aFN + crlf + 'per:' + Format('%5.1f', [t1]) + ' - ' + Format('%5.1f', [t2])
      + crlf + 'delta:' + Format('%4.1f', [delta]) + ' nint:' + IntToStr(nintv);
    // ncoef always 0978 ?
  end
  else
  begin
...
function T_VSOP2013_File.calculate_coordinates(ip: integer; const jde: Double;
{ out: } var Position, Speed: TVector3D_D): boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (jt < 1) or (jt > nintv) then
          raise Exception.Create('invalid coef index'); // sanity check index
    ...
    raise Exception.Create('period outside bounds');
end;
